...
const main = async () => {
  ...
  const tx = await contract.addCollection(collection.id, collection.name, collection.mintConfig, collection.allowlistConfig)
  await tx.wait();
  console.log("Added!")
  ...
  for (let i = 0; i < batches; i++) {
    const tx = await contract.addCollectionDesigns(collection.id,  collection.designs.slice(i*batchSize, i*batchSize+batchSize))
    await tx.wait();
  }
  console.log("Added!")
